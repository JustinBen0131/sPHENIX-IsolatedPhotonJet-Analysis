...
* `train_photon_bdt.py` trains one XGBoost model per collision system from
  photon+jet (signal) and inclusive-jet (background) trees and exports it as a
  TMVA RBDT file plus a manifest (features, label mapping, sample hashes, AUC).
* `score_trees.py` evaluates the model for every candidate with complete
  ...
  regenerates the flags and propagates everything to `photonJets` and
  `eventTree` by candidate identity.  Without `--model` it keeps the stored
  score and only refreshes thresholds and flags.
...
threshold is invalid rather than background.
